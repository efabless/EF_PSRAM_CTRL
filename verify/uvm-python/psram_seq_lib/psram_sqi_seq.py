...
class psram_sqi_seq(psram_base_seq):
    ...
    def __init__(self, name="psram_sqi_seq"):
        super().__init__(name)

    async def body(self):
        await super().body()
        await self.set_mode("SQI")
        # await self.write_bulk(address=0, bulk_size=2, access_size=bus_item.WORD_ACCESS)
        # await self.read_bulk(address=0, bulk_size=2, access_size=bus_item.WORD_ACCESS)
        await self.write_multiple()
        for _ in range(30):
            await self.write_read_multiple()


uvm_object_utils(psram_sqi_seq)
